     ...
     left outer join
       (select
         ...
         where
           ...
           and d.room_name = my_room_name
           and d.category = my_category and d.subcategory = my_subcategory
         group by
           ts_to_bucket (d.timestamp, unit, bucket_size)
       ) g
...
select t.t, '5', '7', 'TEMPERATURE', 'N/A', row_number() over (order by t.t)/200
	from generate_series ('2018-06-07 00:00:00+01'::timestamp with time zone, '2018-06-11 00:00:00+01'::timestamp with time zone, '1 minute'::interval) t;
...
create materialized view test99 as
 SELECT ts_to_bucket(events_all_room."timestamp", 'minute'::text, 15) AS ts_bucket,
  ...
  GROUP BY (ts_to_bucket(events_all_room."timestamp", 'minute'::text, 15)), events_all_room.hc1, events_all_room.hc2, events_all_room.category, events_all_room.subcategory, r.name;
